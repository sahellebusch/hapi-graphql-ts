...
function handleFileUploads(uploadsConfig: FileUploadOptions) {
  return async (request: hapi.Request, _h?: hapi.ResponseToolkit) => {
    if (typeof processFileUploads === 'function' && request.mime === 'multipart/form-data') {
      Object.defineProperty(request, 'payload', {
        value: await processFileUploads(request.raw.req, request.raw.res, uploadsConfig),
        writable: false
      });
    }
  };
  ...
  public async applyMiddleware({
    ...
    await app.ext({
      ...
      method: async function(request, h) {
        ...
        // @ts-ignore TODO: https://github.com/apollographql/apollo-server/pull/3273
